...
import { addMovie, updateMovie } from "../../apis/movies";
...
export default function MovieForm({
  ...
  const onFinish = async(values) => {
    try {
        ...
        let resp = null
        if(formType === "add") {
            resp = await addMovie(values);
        }else {
            console.log(values);
            resp = await updateMovie({...values, _id: selectedMovie._id});
        }
        if(resp.success) {
            fetchMovies();
      ...
      <Form layout="vertical" initialValues={selectedMovie} onFinish={onFinish}>
        <Row gutter={{ xs: 6, sm: 10, md: 12, lg: 16 }}>
          ...
          <Col span={24}>
            ...
            >
              <TextArea placeholder="Enter the decription" />
            </Form.Item>
          </Col>
          <Col span={24}>
            <Row gutter={{ xs: 6, sm: 10, md: 12, lg: 16 }}>
